	...
	if((isobserver(user) || ishuman(user)) && GLOB.fallen_list)
		...
		for(var/i = 1 to length(GLOB.fallen_list))
			if(i != length(GLOB.fallen_list))
				...
				faltext += GLOB.fallen_list[i]
		. += SPAN_NOTICE("To our fallen soldiers: <b>Pfc. Darko Jebach, Cpl. David Taylor, Pfc. Hazel Duncan, Cpl. David Warren, Pvt. Bhim Bahadur Gurung, Cpl. Mordecai Silverburg, Pfc. Reuben O'Mulligan, Cpl. Garreth Vauxhall, Sgt. Muhammed Al-Muradov, Pvt. Joshua Zhang, Cpl. Steve Lancer, Pfc. Arthur Virtanen, Pfc. Dom Blownawei, [faltext]</b>.")
